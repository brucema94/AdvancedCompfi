...
def hull_white(num_paths, vol, maturity, a, dt):
    ...
    dR = (theta(t) - a * interest_rate_paths[:, :-1]) * dt + vol * np.sqrt(dt) * dW
    # Cumulative sum to compute interest rate paths
    interest_rate_paths[:, 1:] = np.cumsum(dR, axis=1) + interest_rate_paths[:, :-1]
    expected_shrotrate = np.mean(interest_rate_paths, axis=0)
...
def NPV(t, short_rate):
    ...
    if len(upcoming_maturities) == 1:
        ...
        NPV = notional*(p_t_T_grid[next_interval] - p_t_T_grid[-1] - fixed*Coupon_frequency*(sum(p_t_T_grid[next_interval:-1]) - p_t_T_grid[0]))
...
def graph_swap_value(N, see_all_paths):
    ...
    #short_rate = hull_white(num_paths, vol, maturity, a, dt)[1]
    #swap_grid = NPV(t_grid[:, np.newaxis], short_rate).T
    #mean_grid = np.mean(swap_grid, axis=0)
    for i in range(N):
        short_rate = hull_white(num_paths, vol, maturity, a, dt)[1]
        print(short_rate)
