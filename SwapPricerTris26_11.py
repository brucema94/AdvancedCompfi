...
def NPV(t, short_rate, maturity_grid):
    if t >= maturity:
        return 0
...
# N is the amount
def graph_swap_value(N, see_all_paths, maturity_grid):
    ...
    for i in range(N):
        ...
        for j in range(len(t_grid)):
            swap_grid[i,j] = NPV(t_grid[j], short_rate, maturity_grid)
...
def graph_EPE(N, final_time, graph, maturity_grid):
    #T = maturity_grid[-1]-dt
    ...
    for i in range(N):
        short_rate = hull_white(vol, maturity, a, dt)
        for j in range(len(t_grid)):
            swap_grid[i,j] = NPV(t_grid[j], short_rate, maturity_grid)*PD_list[j]
        # If you want to see all paths
        #if see_all_paths:
           # plt.plot(t_grid, swap_grid[i, :])
    # Next compute Monte Carlo average
...
def graph_CVA(N, final_maturity):
    # First make sure the maturity actually works
    final_maturity = Coupon_frequency*int(final_maturity/Coupon_frequency)
    finals = [k*Coupon_frequency for k in range(2, 1+int(final_maturity/Coupon_frequency))]
    maturities = []
    CVA_grid = []
    for T in finals:
        maturities.append([j*Coupon_frequency for j in range(1,1+int(T/Coupon_frequency))])
    # Now we will compute the CVA for different final maturities
    print(maturities)
    CVA_grid = [graph_EPE(N, finals[T], False, maturities[T]) for T in range(len(finals))]
    plt.plot(finals, CVA_grid)
    plt.xlim(0.5, final_time)
    plt.xlabel('Maturity $T_m$')
    plt.ylabel('CVA')
    plt.show()
...
#graph_EPE(monte_carlo_amount, final_time, False, maturity_grid)
graph_CVA(monte_carlo_amount, final_time)
